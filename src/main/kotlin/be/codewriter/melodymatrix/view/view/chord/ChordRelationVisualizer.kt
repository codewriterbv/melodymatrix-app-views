...
class ChordRelationVisualizer {
    ...
    companion object {
        ...
        val MOVE_INTERPOLATOR: Interpolator = Interpolator.EASE_BOTH

        fun majorFill(current: Boolean) = if (current) Color.web("#2563EB") else Color.web("#1E3A8A")
        fun minorFill(current: Boolean) = if (current) Color.web("#D97706") else Color.web("#78350F")
        fun dimFill(current: Boolean) = if (current) Color.web("#DC2626") else Color.web("#7F1D1D")
    }
    ...
    fun showChord(chord: Chord) {
        if (chord == Chord.UNDEFINED) return
        if (!isChordEnabled(chord)) {
            clearVisualization()
            return
        }
        enqueueSnapshot(buildSnapshot(chord))
    }
    ...
    private fun buildSnapshot(center: Chord): GraphSnapshot {
        ...
        val related = ChordRelationMap.getRelatedChords(center)
            .filter { rel -> isChordEnabled(rel.chord) }
        ...
        playedTrail.take(MAX_TRAIL_NODES).forEachIndexed { slot, trailChord ->
            if (!isChordEnabled(trailChord)) return@forEachIndexed
    ...
    private fun addInterRelatedEdges(
        ...
        visibleRelated.forEach { source ->
            ...
            ChordRelationMap.getRelatedChords(source.chord)
                .asSequence()
                .filter { candidate -> isChordEnabled(candidate.chord) }
