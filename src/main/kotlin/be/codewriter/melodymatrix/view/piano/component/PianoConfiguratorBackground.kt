...
class PianoConfiguratorBackground : VBox() {
    ...
    init {
        ...
        for (backgroundImage: BackgroundImage in BackgroundImage.entries) {
            ...
            val stackPane = StackPane(imageView).apply {
                background = Background(
                    BackgroundFill(
                        if (backgroundImage == BackgroundImage.NONE) Color.RED else Color.BLACK,
                        CornerRadii.EMPTY,
                        Insets.EMPTY
                    )
                )
            ...
            imageView.setOnMouseClicked {
                (getop<ObjectProperty<*>>(PianoGenerator.PianoProperty.BACKGROUND_IMAGE.name) as ObjectProperty<BackgroundImage>).set(
                    backgroundImage
                )
                removeSelectedImage()
                stackPane.background = Background(BackgroundFill(Color.RED, CornerRadii.EMPTY, Insets.EMPTY))
                imageTransparency.isDisable = backgroundImage == BackgroundImage.NONE
            }
